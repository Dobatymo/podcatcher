...
def playepisode(cast_uid, episode_uid):
	...
	if info:
		...
		try:
			sf = send_file(os.path.join(c.casts_dir, cast_uid, info["localname"]), attachment_filename=filename, mimetype=info["mimetype"], conditional=True, last_modified=info["date"])
		except FileNotFoundError:
			abort(404)
...
def youtube_to_feed(format, url):
	formats = {
		...
		"atom": ("atom_str", "application/atom+xml")
	}
